    ...
    module SuperAdmin
      class LuckyDrawResultsController < ApplicationController
        ...
        def create
          ...
          last_regular  = LuckyDrawResult.where(event_id: @event.id, draw_type: "regular").order(created_at: :desc).first
          window_start  = last_regular&.window_end || @event.created_at
          window_end    = Time.current

          pool = visitors.where(created_at: window_start..window_end).where.not(id: all_won_ids)
        ...
        def bumper
          ...
          if pool.empty?
            return json_error("All #{visitors.count} registered visitor(s) have already won.")
          end
